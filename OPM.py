...
class Environment:
    def __init__(self, OPMFILEDIRECTORY: str, OPMFILENAME: str, startdate = datetime(2023, 1, 1), time_steps = 12, OPMtextoutput = False, 
        ...
        self.name = OPMFILENAME + 'COPY.DATA'
        ...
        if OPMtextoutput:
            self.command = 'flow ' + os.getenv('SLURM_TMPDIR') + '/' +  self.name + ' --enable-opm-rst-file=True'
        else:
            self.command = 'flow ' + os.getenv('SLURM_TMPDIR') + '/' +  self.name + ' --enable-opm-rst-file=True > /dev/null'

    def reset(self):
        '''Create a copy of the OPM input file for each episode/simulation
        '''
        file_copy = open(os.getenv('SLURM_TMPDIR') + '/' + self.OPMFILENAME, 'r').readlines()
        with open(os.getenv('SLURM_TMPDIR') + '/' + self.name, 'w') as copy_file:
    ...
    def step(self, action, step): 
        ...
        Startfile = open(os.getenv('SLURM_TMPDIR') + '/' + self.name, "r")
        ...
        Startfile = open(os.getenv('SLURM_TMPDIR') + '/' + self.name, 'w')
        Startfile.writelines(Startlines[:-1])
        Startfile.close()

        #Append actions to schedule
        with open(os.getenv('SLURM_TMPDIR') + '/' + self.name, 'a') as input:
            # Write the action
            input.write('\n')
    ...
    def get_carbonsats(self):
        ...
        #Open RSM file
        with open(os.getenv('SLURM_TMPDIR') + '/' + self.output, 'r') as file:
