...
@references: Kyubo Noh, Teo Nocita
"""
...
class Environment:
    def __init__(self, OPMFILEDIRECTORY: str, OPMFILENAME: str, process_id: int, startdate = datetime(2023, 1, 1), time_steps = 12, OPMtextoutput = False, 
        ...
        self.OPMFILEDIRECTORY = OPMFILEDIRECTORY

        #Initialize necessary file names 
        self.process_id = process_id
        self.OPMFILENAME = OPMFILENAME + '.DATA'
        self.name = OPMFILENAME + 'COPY' + str(process_id) +  '.DATA'
        ...
        if OPMtextoutput:
            self.command = 'flow ' + self.OPMFILEDIRECTORY + '/' +  self.name + ' --enable-opm-rst-file=True'
        else:
            self.command = 'flow ' + self.OPMFILEDIRECTORY + '/' +  self.name + ' --enable-opm-rst-file=True > /dev/null'

    def reset(self):
        '''Create a copy of the OPM input file for each episode/simulation
        '''
        file_copy = open(self.OPMFILEDIRECTORY + '/' + self.OPMFILENAME, 'r').readlines()
        with open(self.OPMFILEDIRECTORY + '/' + self.name, 'w') as copy_file:
    ...
    def step(self, action, step): 
        ...
        Startfile = open(self.OPMFILEDIRECTORY + '/' + self.name, "r")
        ...
        Startfile = open(self.OPMFILEDIRECTORY + '/' + self.name, 'w')
        Startfile.writelines(Startlines[:-1])
        Startfile.close()

        #Append actions to schedule
        with open(self.OPMFILEDIRECTORY + '/' + self.name, 'a') as input:
            # Write the action
            input.write('\n')
    ...
    def get_carbonsats(self):
        ...
        #Open RSM file
        with open(self.OPMFILEDIRECTORY + '/' + self.output, 'r') as file:
